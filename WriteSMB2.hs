...
fixFileHeader lzData writeOffsets = flip runReaderT 0x0 $ do
  ...
  seekAndDo 0x38 $ fixListOffsetNormal (_cones lzData) (_coneOffset writeOffsets)
  seekAndDo 0x40 $ fixListOffsetNormal (_spheres lzData) (_sphereOffset writeOffsets)
  seekAndDo 0x48 $ fixListOffsetNormal (_cylinders lzData) (_cylinderOffset writeOffsets)
...
fixCollisionHeaders lzData writeOffsets = do
  ...
  fixList collisionHeaderOffset 0x49c pairsOfOffsets $
    \(collHdr,animHdrOff,collTriOff,indexPointerListOff) -> do
      ...
      seekAndDo 0x64 $ fixListOffsetForCollHeader (_coneOffset writeOffsets) (_coneListOffset collHdr)
      seekAndDo 0x6c $ fixListOffsetForCollHeader (_sphereOffset writeOffsets) (_sphereListOffset collHdr)
      seekAndDo 0x74 $ fixListOffsetForCollHeader (_cylinderOffset writeOffsets) (_cylinderListOffset collHdr)
...
writeSMB2 lzData = do
  ...
  writeOffsets <- pure WriteOffsets
    ...
    <*> writeObjects (_cones lzData)
    <*> writeObjects (_spheres lzData)
    <*> writeObjects (_cylinders lzData)
